...
func (e *executorImpl) buildSearchRanges(plan *planImpl) []keyRange {
	...
	kinds := plan.filter.Kinds
	if len(kinds) == 0 {
		// No kinds available — cannot construct per-kind search ranges.
		// This means the query compiler did not inject kinds (empty database
		// or no DefaultKindsFunc configured). Return empty ranges.
		return ranges
	}
